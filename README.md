...
| `--extra-dir CHEMIN` | Répertoire supplémentaire à inspecter. Répétable. |
...
- **Élévation de privilèges effective** (RUID ≠ EUID = 0) et **processus tracé** (`TracerPid` ≠ 0, injection en cours).
...
**Bornes de sûreté** : profondeur maximale de 6 niveaux, budget global de 60 000 fichiers, plafond par racine, aucun franchissement de point de montage (les partages réseau ne sont jamais parcourus), symlinks jamais suivis, moteur regex borné à 64 Ko par fichier.
...
- **Aucune vérification d'intégrité de paquets.** Le script ne compare pas les binaires système à leur empreinte de distribution (pas de `dpkg -V` / `rpm -Va`, qui supposeraient un binaire externe).
